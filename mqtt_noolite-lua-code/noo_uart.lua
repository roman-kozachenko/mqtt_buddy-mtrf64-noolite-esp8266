...
function noo_uart.start(log)
    ...
    uart.on("data", 17,
        function(data)
            ...
            log('mtrf64> received RX '..rx_a..rx_b)

            -- answer_code=3 means bind success
            if answer_code == '3' then  
                log("mqtt_buddy/sys", 'mtrf64> BINDING. chan '..channel..' addr '..id1..':'..id2..':'..id3..':'..id4)
            ...
            elseif LAST_ACTION == 'unbind' and (data3 == '192' or data3 == '193') then
                log("mqtt_buddy/sys", 'mtrf64> UNBINDING> chan '..channel..' addr '..id1..':'..id2..':'..id3..':'..id4)
